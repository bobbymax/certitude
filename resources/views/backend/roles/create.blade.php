	...
	<div class="card mb-30">
		...
		<div class="card-body">
			<form action="{{ route('roles.store') }}" method="POST">
						...
						<div class="form-group">
							...
							<select name="ranking" class="form-control @error('ranking') is-invalid @enderror" id="ranking">
								...
								@foreach ($rankings as $value)
									<option value="{{ $value }}" {{ old('ranking') == $value ? ' selected' : '' }}>{{ ucfirst($value) }}</option>
								@endforeach
							</select>
							@error('ranking')
								<span class="invalid-feedback" role="alert">
				...
				<a href="{{ route('roles.index') }}" class="btn btn-sm btn-warning"><i class="bx bx-arrow-back mr-2"></i>Go Back</a>
			</form>
		</div>
	</div>
